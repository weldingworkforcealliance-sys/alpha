...
export default function ClassTimeWatchdog() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const timings = new Map<string, { id: string; start_time: string | null; end_time: string | null }>(
        (timingResult.data ?? []).map((row: { id: string; start_time: string | null; end_time: string | null }) => [
          row.id,
          row,
        ])
      );
      const deliveries = new Map<string, { planner_day_id: string; delivery_status: string | null; started_at: string | null; completed_at: string | null }>(
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                >
                  {alert.courseLabel ? `Open ${alert.courseLabel}` : 'Open Class'}
                </a>
                {alert.severity !== 'start' && (
                  <a
