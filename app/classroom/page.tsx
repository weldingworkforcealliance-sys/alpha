...
export default function ClassroomPage() {
  ...
  useEffect(()=>{
    (async()=>{
      try{
        ...
        // Cleanup is useful, but it must never prevent the classroom from opening.
        try{
          await expireClassroomSessions(supabase);
        }catch(cleanupError){
          console.warn('Live Classroom cleanup failed:',formatError(cleanupError));
        }
        ...
        if(se)throw new Error(`Unable to load classes: ${formatError(se)}`);
        if(ae)throw new Error(`Unable to load assessments: ${formatError(ae)}`);
  ...
  useEffect(()=>{
    ...
    QRCode.toDataURL(joinUrl,{width:340,margin:2,color:{dark:'#050505',light:'#ffffff'}}).then(setQr).catch(e=>setError(formatError(e)));
  ...
  const openReport=async(id:string)=>{
    ...
    setBusy(false);
    if(e)setError(formatError(e));else setReport(data as Report);
  };
  ...
  const openAnswerKey=async(slug=assessmentSlug)=>{
    ...
    setBusy(false);
    if(e)setError(formatError(e));else setAnswerKey(data as AnswerKey);
  };
  ...
  return <div className="shell">
    <header><div><div className="eyebrow">Living Teacher Guide</div><h1>Connected Classroom Testing</h1></div><div className="header-actions"><button className="job-card-entry" onClick={()=>router.push(jobCardUrl)}>Live Job Card</button><button onClick={()=>router.push('/dashboard')}>Back to Planner</button></div></header>
    <main>
      {error&&<div className="error"><span>{error}</span><button type="button" onClick={()=>window.location.reload()}>Retry</button></div>}
