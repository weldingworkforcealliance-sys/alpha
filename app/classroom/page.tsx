...
export default function ClassroomPage() {
    ...
    (async()=>{
      try{
        ...
        if(sectionIds.length&&(!hasRequestedContext||requestedContextIsValid)){
          ...
          if(restored){
            setSession(restored);setSectionId(restored.section_id);setAssessmentSlug(restored.assessment_slug);setExpectedStudents(restored.expected_students);setGradeCategory(restored.grade_category);
          }
        }
  ...
  const start=async()=>{
    ...
    try{
      const created=await createClassroomSession(supabase,{sectionId,assessmentSlug,expectedStudents,gradeCategory});
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {!session?<section className="panel start">
        ...
        <p>Choose the class, assessment, and how this session should be used in Student Progress. Practice Only is the safe default and does not affect averages.</p>
        <div className="setup-grid">
          ...
          <label>Student Progress Use<select value={gradeCategory} onChange={e=>setGradeCategory(e.target.value as GradeCategory)}>{GRADE_CATEGORY_OPTIONS.map(option=><option key={option.value} value={option.value}>{option.label}{option.counts?' · averaged':' · evidence only'}</option>)}</select></label>
        </div>
        <div className="gradebook-note"><strong>{gradeCategoryLabel(gradeCategory)}</strong><span>{GRADE_CATEGORY_OPTIONS.find(option=>option.value===gradeCategory)?.counts?'This session will feed the selected category average.':'This session will be saved as evidence but will not affect grade averages.'}</span></div>
