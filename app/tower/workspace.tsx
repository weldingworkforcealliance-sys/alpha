...
export default function TowerWorkspace(){
 ...
 useEffect(()=>{let alive=true;void readGradebookRows<Gradebook>((from,to)=>client.from('gradebook_directory').select('*').eq('course_role','lab').eq('section_status','active').order('section_name').order('id').range(from,to))
  .then(rows=>{if(alive){setBooks(rows);setSelected(rows[0]?.id||'');setLoading(false);}}).catch(c=>{if(alive){setError(formatError(c,'Unable to load your assigned classes.'));setLoading(false);}});
 ...
 useEffect(()=>{
  async function receive(e:MessageEvent){
   ...
   if(m?.type==='tower-ready'){target?.postMessage({type:'tower-init',payload:current.current},window.location.origin);return;}
 ...
 return <main><h1>Welding Record Tower</h1><p>Class rosters · weld grades · competencies · permanent test records</p>
