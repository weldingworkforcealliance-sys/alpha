...
export default function ShopWorkspace({gradebookId,onSaveState}:{gradebookId:string;onSaveState?:(blocked:boolean)=>void}) {
 ...
 const [history,setHistory]=useState<string|null>(null),[notice,setNotice]=useState(''),[link,setLink]=useState('');
 ...
 async function practice(){
  ...
  try{
   ...
   if(result.error)throw result.error;accept(result.data);setCoaching(null);setNotice('Practice focus saved. No grade recorded.');
  }catch(e){setError(formatError(e));}finally{saving.current=false;setBusy(false);}
 }
 async function studentLink(student:ShopStudent){
  if(saving.current)return;
  saving.current=true;setBusy(true);setError('');setLink('');
  try{
   const result=await client.rpc('issue_wld110_student_link',{p_gradebook_id:gradebookId,p_student_id:student.student_id});
   if(result.error)throw result.error;
   setLink(window.location.origin+'/shop/student#'+result.data);
   setNotice('Personal shop link for '+student.display_name+'. This replaces their previous link.');
  }catch(e){setError(formatError(e));}finally{saving.current=false;setBusy(false);}
 }
 ...
 return <div className={styles.shop}>
  ...
  {link&&<p><a href={link} target="_blank" rel="noreferrer">Open personal shop card</a> <button onClick={()=>void navigator.clipboard.writeText(link).then(()=>setNotice('Personal shop link copied.')).catch(()=>setError('Copy failed. Use your browser’s Copy link address action on Open personal shop card.'))}>Copy student link</button></p>}
  ...
  {board&&<div className={styles.card+' '+styles.scroll}><table><caption>Check queue and current work</caption><thead><tr><th>Student</th><th>Current work</th><th>Status / focus</th><th>Action</th></tr></thead><tbody>
   {students.map(s=><tr key={s.student_id}><td>{s.display_name}{!s.active?' (inactive)':''}</td><td>{assignmentLabel(s.current_competency)}<br/><small>Core {Math.min(s.current_competency,8)} / 8</small></td>
    ...
    <td><div className={styles.actions}><button className={styles.primary} disabled={busy||Boolean(selected||coaching)||!s.active||s.current_competency===9} onClick={()=>{setSelected(s);setNotice('');setLink('');setError('');}}>Grade weld</button>
     <button disabled={busy||Boolean(selected||coaching)||!s.active||s.current_competency===9} onClick={()=>{setCoaching(s);setFocus(s.focus.slice(0,12));setError('');}}>Practice / coach</button>
     <button disabled={busy||Boolean(selected||coaching)||!s.active} onClick={()=>void studentLink(s)}>Student link</button>
