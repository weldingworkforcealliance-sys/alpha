...
function isPcccName(name: string | null | undefined) {
  ...
  return normalized === 'pccc' || normalized.includes('passaic county community college') || normalized.includes('pccc welding');
}
...
function clearPcccClasses() {
  ...
  document.body.classList.remove('pccc-welding-skin', 'pccc-school-skin', 'pccc-instructor-skin', 'pccc-demo-skin');
  ...
  useEffect(() => {
    ...
    const apply = async () => {
      ...
      try {
        ...
        const { data: schools, error: schoolError } = await supabase.from('schools').select('id,name').in('id', schoolIds);
        if (schoolError || cancelled) return;

        const pcccSchoolIds = new Set(((schools ?? []) as School[]).filter((school) => isPcccName(school.name)).map((school) => school.id));
        const pcccMembership = rows.find((row) => pcccSchoolIds.has(row.school_id));
        if (!pcccMembership || cancelled) return;

        const roleUsesSchoolPortal = Boolean(pcccMembership.role && SCHOOL_ACCESS_ROLES.has(pcccMembership.role));
        const schoolAccess = roleUsesSchoolPortal || isSchoolContext(pathname);
        document.body.classList.add('pccc-welding-skin', schoolAccess ? 'pccc-school-skin' : 'pccc-instructor-skin');
