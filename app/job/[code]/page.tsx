...
export default function StudentLiveJobCardPage() {
  ...
  const setRequirementCheck = (key: string, value: RequirementCheck) => {
    ...
    if (value === 'na') {
      setActualValues((current) => ({ ...current, [key]: '' }));
    }
  };
  ...
  if (error && !payload) {
    return <main className="center"><div className="card"><h1>Unable to Open Job Card</h1><p className="error-text">{error}</p></div><style jsx>{styles}</style></main>;
  }
  if (!session) return <main className="center">Opening Live Job Card…<style jsx>{styles}</style></main>;
  if (submitted) {
    return <main className="center"><div className="card complete"><div className="check">✓</div><h1>Job Card Submitted</h1><p>Your instructor received your work record and will make the final review decision.</p></div><style jsx>{styles}</style></main>;
  }
      ...
      <main className="center">
        <div className="card intro">
          ...
          <div className="job-summary">
            <strong>{session.job_header.jobPlannerDay}</strong>
            <span>{session.job_header.processPosition}</span>
            <span>{session.job_header.materialJoint}</span>
          </div>
  ...
  return (
    <main>
      <header className="student-top">
        <div><div className="eyebrow">Live Job Card</div><h1>{name}</h1></div>
        <div className="timer">3–5 min entry</div>
      </header>
      ...
      <section className="card section">
        ...
        <div className="identity-strip">
          <div><span>Student</span><strong>{name}</strong></div>
          <div><span>Date</span><strong>{new Date(session.started_at).toLocaleDateString()}</strong></div>
          <div><span>Class</span><strong>{session.class_label}</strong></div>
        </div>
      ...
      <section className="card section">
        ...
        {session.requirements.map((requirement) => (
          <div className="requirement-row" key={requirement.key}>
            ...
            <span>{requirement.required}</span>
            ...
            <div className="check-buttons">
              {(['pass', 'correct', 'na'] as RequirementCheck[]).map((value) => (
                <button type="button" key={value} className={requirementChecks[requirement.key] === value ? 'check-option active' : 'check-option'} onClick={() => setRequirementCheck(requirement.key, value)}>{value === 'pass' ? 'Pass' : value === 'correct' ? 'Correct' : 'N/A'}</button>
              ))}
            </div>
          </div>
        ))}
      ...
      <section className="card section">
        ...
        <div className="check-grid">
          {session.start_checks.map((check) => (
            <label className={startConfirmations[check] ? 'tick checked' : 'tick'} key={check}>
              <input type="checkbox" checked={Boolean(startConfirmations[check])} onChange={(event) => setStartConfirmations((current) => ({ ...current, [check]: event.target.checked }))} />
              <span>{check}</span>
            </label>
          ))}
        </div>
      ...
      <section className="card section">
        ...
        <div className="quality-layout">
          <div className="check-grid one">
            {session.quality_checks.map((check) => (
              <label className={qualityConfirmations[check] ? 'tick checked' : 'tick'} key={check}>
                <input type="checkbox" checked={Boolean(qualityConfirmations[check])} onChange={(event) => setQualityConfirmations((current) => ({ ...current, [check]: event.target.checked }))} />
                <span>{check}</span>
              </label>
            ))}
          </div>
          <div className={needsCorrection ? 'correction active' : 'correction'}>
            ...
            <div className="recheck">
              <button type="button" disabled={!needsCorrection} className={recheckStatus === 'pass' ? 'check-option active' : 'check-option'} onClick={() => setRecheckStatus('pass')}>Recheck PASS</button>
              <button type="button" disabled={!needsCorrection} className={recheckStatus === 'needs_more_work' ? 'check-option active' : 'check-option'} onClick={() => setRecheckStatus('needs_more_work')}>Needs more work</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="card section">
        ...
        <div className="evidence-options">
          {EVIDENCE_OPTIONS.map((item) => (
            <button type="button" className={evidenceTypes.includes(item.value) ? 'evidence active' : 'evidence'} key={item.value} onClick={() => toggleEvidence(item.value)}>{item.label}</button>
          ))}
        </div>
