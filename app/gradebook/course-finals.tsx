...
export default function CourseFinals({bookId,students}:{bookId:string;students:{student_id:string;display_name:string;active:boolean}[]}){
 ...
 async function review(){
  ...
  try{const [p,f]=await Promise.all([client.rpc('preview_gradebook_final',{p_gradebook_id:bookId,p_student_id:student}),client.from('gradebook_finalizations').select('id,snapshot,source_fingerprint,reason,finalized_at').eq('gradebook_id',bookId).eq('student_id',student).order('revision',{ascending:false})]);if(p.error)throw p.error;if(f.error)throw f.error;setPreview(p.data);setFinals(f.data||[]);}catch(e){setError(formatError(e,'Could not load course totals.'));}finally{setBusy(false);}
 }
 ...
 <label>Student for final grade<select disabled={busy} value={student} onChange={e=>{setStudent(e.target.value);setPreview(null);setFinals([]);}}><option value="">Choose student</option>{students.map(s=><option key={s.student_id} value={s.student_id}>{s.display_name}{!s.active?' (inactive)':''}</option>)}</select></label>
