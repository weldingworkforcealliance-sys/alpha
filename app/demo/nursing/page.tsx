...
const DEMO_DAYS: DemoDay[] = [
  ...
  },
  {
    ...
    demonstration: 'Instructor models a focused assessment simulation sequence and narrates what is observed, verified, and documented.',
  ...
  },
  {
    ...
    checkoff: 'Instructor records Ready, Needs Coaching, or Recheck Required for each competency area and documents next steps.',
...
const QUALITY_ROWS = [
  ['OUT-1', 'Skills observation', 'Day 1, 3, 7', 'Evidence captured'],
  ['OUT-2', 'Instructor rubric', 'Day 3, 6, 7', 'Evidence captured'],
  ['OUT-3', 'Documentation review', 'Day 2, 4, 5, 7', 'Evidence captured'],
  ['OUT-4', 'Safety-gate observation', 'Day 1, 3, 4, 7', 'Evidence captured'],
];
  ...
  return (
    <div className="shell">
      <div className="demo-banner">
        NURSING DEMONSTRATION · SYNTHETIC DATA ONLY · NOT AN APPROVED NURSING CURRICULUM
      </div>
      ...
      <header>
        ...
        <div className="actions">
          <button onClick={() => router.push('/demo')}>View Welding Demo</button>
      ...
      <main>
        <section className="identity-card">
          <div className="identity-title">
            <span>LTG Demonstration Institute</span>
            <strong>Health Sciences · Nursing</strong>
          </div>
          <div className="identity-meta">
            <div><span>Course</span><strong>NUR 101</strong><small>Fundamentals of Nursing Practice</small></div>
            <div><span>Section</span><strong>Cohort A</strong><small>8 synthetic learners</small></div>
            <div><span>Learning Environment</span><strong>Classroom + Skills Lab</strong><small>Simulation-based demo</small></div>
            <div><span>Curriculum Status</span><strong>Protected</strong><small>Demo v1.0</small></div>
          </div>
        </section>
        ...
        <section className="value-strip">
          ...
          <ValueCard label="Attendance" value={`${recordedCount} / 8 recorded`} detail="Section-level student record" />
        ...
        {view === 'instructor' && (
          <div className="instructor-layout">
            ...
            <section className="panel guide-panel">
              ...
              <GuideRow label="Competency Evidence" text={day.checkoff} />
              ...
              <div className="protected-box">
                <div className="lock">PROTECTED CURRICULUM</div>
                <strong>Approved outcomes remain locked.</strong>
                <span>Faculty can improve pacing, resources, demonstrations, sequencing, notes, and remediation guidance without silently rewriting approved outcomes.</span>
              </div>

              <div className="checkoff-box">
                <div className="section-title-row">
                  <div>
                    <div className="eyebrow">Skills Evidence</div>
                    <h3>Competency Validation Snapshot</h3>
                  </div>
                  <span className="muted">Demo statuses</span>
                </div>
                {Object.entries(checkoffs).map(([skill, value]) => (
                  <div className="checkoff-row" key={skill}>
                    <span>{skill}</span>
                    <select value={value} onChange={(event) => setCheckoffs((current) => ({ ...current, [skill]: event.target.value as SkillStatus }))}>
                      <option>Ready</option>
        ...
        {view === 'school' && (
          <div className="school-layout">
            <section className="panel">
              <div className="eyebrow">Program Administration</div>
              <h2>Health Sciences · Nursing</h2>
              <p className="lead">One view of section activity, attendance completion, curriculum control, instructor observations, and skills follow-up.</p>
              <div className="metrics">
                ...
                <Metric label="Skills Follow-up" value={String(coachingCount + recheckCount)} />
              </div>
            ...
            <section className="panel two-column">
              <div>
                ...
                <h3>What program leadership approves</h3>
        ...
        {view === 'quality' && (
          <div className="quality-layout">
            <section className="panel quality-hero">
              <div>
                <div className="eyebrow">Quality & Evidence</div>
                <h2>Show where learning evidence is being produced.</h2>
                <p className="lead">LTG can connect approved local outcomes to daily instruction, skills evidence, faculty observations, attendance, and improvement records without changing the institution's curriculum governance process.</p>
              </div>
              <div className="quality-summary">
                <Metric label="Protected Outcomes" value="4" />
                <Metric label="Evidence Types" value="4" />
                <Metric label="Demo Days Mapped" value="7" />
              </div>
            </section>

            <section className="panel">
              <div className="eyebrow">Outcome Evidence Matrix</div>
              <h3>Local outcomes connected to observable evidence</h3>
              <div className="quality-table">
                <div className="quality-head"><span>Outcome</span><span>Evidence Type</span><span>Instructional Days</span><span>Status</span></div>
                {QUALITY_ROWS.map(([outcome, evidence, days, status]) => (
                  <div className="quality-row" key={outcome}><b>{outcome}</b><span>{evidence}</span><span>{days}</span><em>{status}</em></div>
                ))}
              </div>
            </section>
            ...
            <section className="quality-columns">
              <div className="panel">
                <div className="eyebrow">External Framework Mapping</div>
                <h3>Configurable, institution-controlled references</h3>
                <p className="lead">A nursing program could map its own approved curriculum to frameworks or regulatory references it is authorized to use.</p>
                <div className="framework-list">
                  <div><strong>AACN Essentials</strong><span>External reference example</span></div>
                  <div><strong>ACEN standards</strong><span>External reference example</span></div>
                  <div><strong>State Board of Nursing requirements</strong><span>Institution-supplied mapping</span></div>
                  <div><strong>Program-specific competencies</strong><span>Local approved outcomes</span></div>
                </div>
                <div className="notice">The demo does not reproduce or claim endorsement by any external standards organization. Institutions control licensed or authorized reference content.</div>
              </div>

              <div className="panel">
                <div className="eyebrow">Evidence Trail</div>
                <h3>What a reviewer could follow</h3>
                <div className="timeline">
                  <div><b>1</b><span><strong>Approved outcome</strong><small>Locked at the program level</small></span></div>
                  <div><b>2</b><span><strong>Daily instruction</strong><small>Planner shows where the outcome is taught</small></span></div>
                  <div><b>3</b><span><strong>Student evidence</strong><small>Attendance, checkoff, rubric, or assessment</small></span></div>
                  <div><b>4</b><span><strong>Faculty observation</strong><small>Implementation note or coaching need</small></span></div>
                  <div><b>5</b><span><strong>Program review</strong><small>Accepted improvements feed future planning</small></span></div>
                </div>
              </div>
            </section>
          </div>
        ...
        {view === 'platform' && (
          <div className="platform-layout">
            <section className="panel hero-proof">
              ...
              <h2>Welding is one implementation. Nursing is another.</h2>
              <p className="lead">The core operating model is unchanged: organization, program, course, section, daily planner, protected outcomes, attendance, instructor notes, competency evidence, and reporting.</p>
            ...
            <section className="comparison-grid">
              <div className="panel program-card welding-card">
                <span className="program-tag">IMPLEMENTATION 01</span>
                <h3>Welding Technology</h3>
                <p>Shop instruction, welding procedures, fabrication activities, performance testing, attendance, timekeeping, and daily planner operations.</p>
                <button onClick={() => router.push('/demo')}>Open Welding Demo</button>
              </div>
              <div className="panel program-card nursing-card">
                <span className="program-tag">IMPLEMENTATION 02</span>
                <h3>Nursing</h3>
                <p>Classroom and skills-lab instruction, competency evidence, checkoffs, attendance, curriculum protection, and program-quality review.</p>
                <button className="solid" onClick={() => setView('instructor')}>Open Nursing Workspace</button>
              </div>
            </section>

            <section className="panel">
              <div className="eyebrow">Terminology Layer</div>
              <h3>Same platform functions, different instructional language</h3>
              <div className="terminology-table">
                <div className="term-head"><span>Welding Implementation</span><span>Nursing Implementation</span></div>
      ...
      <style jsx>{`
        .shell { min-height:100vh; background:#f4f8fa; color:#20313a; }
        .demo-banner { position:sticky; top:0; z-index:30; padding:9px 14px; text-align:center; background:#0e6c88; color:#fff; font-size:10px; font-weight:900; letter-spacing:.08em; }
        header { display:flex; justify-content:space-between; align-items:flex-start; gap:22px; padding:26px 32px; border-bottom:1px solid #d8e4e9; background:#ffffff; }
        .brand-block { display:flex; gap:15px; align-items:flex-start; }
        .brand-mark { width:52px; height:52px; display:grid; place-items:center; border-radius:14px; background:#0e6c88; color:white; font-weight:950; letter-spacing:.04em; box-shadow:0 8px 18px rgba(14,108,136,.18); }
        h1,h2,h3 { color:#17313c; margin:5px 0; } h1 { font-size:29px; } h2 { font-size:22px; } h3 { font-size:16px; }
        .subtitle,.lead { color:#687b85; max-width:820px; margin:7px 0 0; line-height:1.55; font-size:13px; }
        .eyebrow { color:#0e7898; font-size:10px; text-transform:uppercase; letter-spacing:.12em; font-weight:900; }
        .actions, nav, .controls, .day-nav { display:flex; gap:8px; flex-wrap:wrap; }
        button, select { border:1px solid #c9d9e0; background:#fff; color:#34515e; border-radius:8px; padding:9px 12px; font-weight:800; }
        button { cursor:pointer; } button:hover:not(:disabled), button.active { border-color:#0e7898; color:#0e7898; background:#f2fbfe; }
        button.primary { border-color:#16865f; color:#16865f; background:#f2fbf7; } button.solid { border-color:#0e6c88; background:#0e6c88; color:white; } button.solid:hover { color:white; background:#0a5b73; }
        button:disabled { opacity:.42; cursor:not-allowed; }
        main { width:min(1420px, calc(100% - 30px)); margin:auto; padding:20px 0 56px; }
        .identity-card { display:grid; grid-template-columns:.85fr 2.15fr; overflow:hidden; border:1px solid #d5e3e8; border-radius:13px; background:white; box-shadow:0 10px 28px rgba(44,83,99,.06); margin-bottom:12px; }
        .identity-title { padding:20px; background:linear-gradient(135deg,#0e6c88,#2189a4); color:white; display:grid; align-content:center; gap:4px; } .identity-title span { font-size:10px; text-transform:uppercase; letter-spacing:.1em; opacity:.75; font-weight:850; } .identity-title strong { font-size:19px; }
        .identity-meta { display:grid; grid-template-columns:repeat(4,1fr); } .identity-meta div { padding:16px; border-left:1px solid #e3ecef; } .identity-meta span,.identity-meta small { display:block; } .identity-meta span { color:#84949c; font-size:9px; text-transform:uppercase; font-weight:850; letter-spacing:.06em; } .identity-meta strong { display:block; color:#1f3944; font-size:13px; margin-top:4px; } .identity-meta small { color:#71828a; font-size:10px; margin-top:3px; }
        .value-strip { display:grid; grid-template-columns:repeat(4,1fr); gap:10px; margin-bottom:16px; } .value-card { padding:13px 15px; background:#fff; border:1px solid #d8e4e9; border-radius:10px; } .value-card span,.value-card small { display:block; } .value-card span { color:#7f9098; text-transform:uppercase; font-size:9px; font-weight:850; letter-spacing:.05em; } .value-card strong { display:block; color:#17313c; margin-top:3px; font-size:16px; } .value-card small { color:#74858d; margin-top:3px; }
        nav { border-bottom:1px solid #d5e2e7; padding:0 0 12px; margin-bottom:16px; }
        .instructor-layout { display:grid; grid-template-columns:minmax(380px,.9fr) minmax(500px,1.1fr); gap:16px; }
        .school-layout,.quality-layout,.platform-layout { display:grid; gap:16px; }
        .panel { background:#fff; border:1px solid #d7e4e9; border-radius:12px; padding:20px; box-shadow:0 8px 24px rgba(44,83,99,.06); }
        .panel-head,.section-title-row { display:flex; justify-content:space-between; align-items:flex-start; gap:14px; }
        .status-pill { border:1px solid #c7d8df; border-radius:999px; padding:7px 10px; color:#71838b; font-size:9px; font-weight:900; letter-spacing:.08em; background:#f8fbfc; } .status-pill.on { border-color:#16865f; color:#16865f; background:#f2fbf7; }
        ...
        .metric { background:#f7fafb; border:1px solid #e0eaee; border-radius:9px; padding:11px; } .metric span { display:block; color:#7f9098; font-size:9px; text-transform:uppercase; font-weight:850; } .metric strong { display:block; color:#17313c; margin-top:4px; font-size:18px; }
        .controls { margin:14px 0 18px; }
        .attendance-card,.resource-box,.checkoff-box,.protected-box,.saved-note { background:#f8fbfc; border:1px solid #dbe7eb; border-radius:9px; padding:14px; margin-top:14px; }
        .muted { color:#7e8f97; font-size:10px; }
        .attendance-grid { display:grid; grid-template-columns:1fr 1fr; gap:7px; margin-top:10px; }
        .student-row { display:flex; justify-content:space-between; align-items:center; gap:8px; background:#fff; border:1px solid #e0e9ed; border-radius:8px; padding:8px; }
        .student-row strong,.student-row span { display:block; } .student-row strong { font-size:12px; color:#29434f; } .student-row span { color:#87969d; font-size:9px; margin-top:2px; } .student-row select { padding:6px 8px; font-size:10px; }
        label { display:grid; gap:7px; margin-top:14px; color:#74868e; font-size:10px; font-weight:850; text-transform:uppercase; letter-spacing:.05em; }
        textarea { min-height:96px; resize:vertical; background:#fff; color:#28414d; border:1px solid #cadbe2; border-radius:8px; padding:11px; font:inherit; }
        .saved-note { display:grid; gap:4px; } .saved-note strong { color:#0e7898; font-size:10px; text-transform:uppercase; } .saved-note span { color:#61747d; font-size:12px; line-height:1.5; }
        .day-nav { margin:10px 0 14px; } .day-nav button { padding:7px 9px; font-size:10px; }
        .guide-row { display:grid; gap:5px; padding:12px 0; border-bottom:1px solid #e5edef; } .guide-row strong { color:#0e7898; font-size:9px; text-transform:uppercase; letter-spacing:.09em; } .guide-row span { color:#50646d; line-height:1.55; font-size:12px; }
        .resource-box { color:#536973; font-size:11px; line-height:1.65; border-color:#c9e2eb; background:#f3fbfe; }
        .protected-box { display:grid; gap:5px; border-color:#ead4a9; background:#fffaf0; } .protected-box .lock { color:#a96714; font-size:9px; font-weight:900; letter-spacing:.12em; } .protected-box span { color:#75694f; font-size:11px; line-height:1.5; }
        .checkoff-row { display:grid; grid-template-columns:1fr 170px; gap:9px; align-items:center; padding:8px 0; border-bottom:1px solid #e4edef; font-size:11px; } .checkoff-row:last-child { border-bottom:0; } .checkoff-row select { font-size:10px; padding:7px; }
        .table { border:1px solid #d9e5e9; border-radius:9px; overflow:hidden; margin-top:16px; } .table-head,.table-row { display:grid; grid-template-columns:1.5fr 1fr .8fr 1fr .8fr; gap:10px; padding:11px 12px; } .table-head { background:#f4f8fa; color:#7f9098; font-size:9px; text-transform:uppercase; font-weight:850; } .table-row { border-top:1px solid #dfe9ed; color:#52666f; font-size:11px; }
        .two-column { display:grid; grid-template-columns:1.25fr .75fr; gap:22px; }
        .outcome { display:grid; grid-template-columns:56px 1fr 52px; gap:10px; align-items:start; padding:10px 0; border-bottom:1px solid #e5edef; } .outcome b { color:#0e7898; font-size:9px; } .outcome span { color:#50636d; font-size:11px; line-height:1.45; } .outcome em { color:#a96714; font-size:9px; font-style:normal; text-transform:uppercase; font-weight:850; }
        .scope-list { display:grid; gap:9px; margin-top:12px; color:#50636d; font-size:11px; } .scope-list .blocked { color:#a64949; margin-top:5px; }
        .quality-hero { display:flex; justify-content:space-between; align-items:flex-start; gap:22px; background:linear-gradient(120deg,#fff,#f1fbfe); } .quality-summary { display:grid; grid-template-columns:repeat(3,145px); gap:8px; }
        .quality-columns { display:grid; grid-template-columns:1fr 1fr; gap:16px; }
        .quality-table { border:1px solid #d9e5e9; border-radius:9px; overflow:hidden; margin-top:14px; } .quality-head,.quality-row { display:grid; grid-template-columns:.6fr 1.2fr 1fr .8fr; gap:10px; padding:11px 12px; } .quality-head { background:#f4f8fa; color:#7d8e96; font-size:9px; text-transform:uppercase; font-weight:850; } .quality-row { border-top:1px solid #e1e9ed; font-size:11px; color:#50636d; } .quality-row b { color:#0e7898; } .quality-row em { font-style:normal; color:#16865f; font-weight:800; }
        .framework-list { display:grid; gap:8px; margin-top:14px; } .framework-list div { display:flex; justify-content:space-between; gap:12px; border:1px solid #dce7eb; border-radius:8px; padding:10px; background:#f8fbfc; } .framework-list strong { color:#28434f; font-size:11px; } .framework-list span { color:#819098; font-size:10px; text-align:right; }
        .notice { margin-top:12px; padding:11px; border-radius:8px; background:#fff8e9; border:1px solid #ecd9ad; color:#776846; font-size:10px; line-height:1.5; }
        .timeline { display:grid; gap:9px; margin-top:14px; } .timeline div { display:grid; grid-template-columns:30px 1fr; gap:10px; align-items:center; } .timeline b { width:30px; height:30px; display:grid; place-items:center; border-radius:50%; background:#e8f6fa; color:#0e7898; } .timeline span,.timeline strong,.timeline small { display:block; } .timeline strong { color:#29434f; font-size:11px; } .timeline small { color:#829199; margin-top:2px; }
        .hero-proof { background:linear-gradient(120deg,#fff,#eef9fc); } .architecture { display:flex; flex-wrap:wrap; gap:8px; align-items:center; margin-top:18px; } .architecture span { border:1px solid #c9dfe7; background:#fff; border-radius:8px; padding:9px 11px; color:#33545f; font-size:10px; font-weight:800; } .architecture b { color:#8da1aa; }
        .comparison-grid { display:grid; grid-template-columns:1fr 1fr; gap:16px; } .program-card { min-height:190px; display:grid; align-content:start; gap:7px; } .program-card p { color:#687b85; line-height:1.55; font-size:12px; margin:0 0 10px; } .program-tag { font-size:9px; font-weight:900; letter-spacing:.1em; color:#7f9098; } .welding-card { border-top:4px solid #38444a; } .nursing-card { border-top:4px solid #0e7898; }
        ...
        .feature-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; margin-top:12px; } .feature { border:1px solid #dbe6ea; border-radius:8px; background:#f8fbfc; padding:11px; color:#52666f; font-size:11px; }
        @media(max-width:1050px) { .identity-card { grid-template-columns:1fr; } .identity-meta { grid-template-columns:1fr 1fr; } .instructor-layout,.two-column,.quality-columns { grid-template-columns:1fr; } .metrics,.metrics.compact { grid-template-columns:repeat(3,1fr); } .feature-grid { grid-template-columns:1fr 1fr; } .quality-hero { display:grid; } }
        @media(max-width:720px) { header { flex-direction:column; padding:20px 16px; } .brand-block { align-items:center; } .brand-mark { width:44px; height:44px; } h1 { font-size:24px; } main { width:min(100% - 18px,1420px); } .identity-meta,.value-strip,.attendance-grid,.metrics,.metrics.compact,.feature-grid,.comparison-grid,.quality-summary { grid-template-columns:1fr; } .table-head,.quality-head { display:none; } .table-row,.quality-row { grid-template-columns:1fr; } .checkoff-row { grid-template-columns:1fr; } }
      `}</style>
    </div>
  );
}
