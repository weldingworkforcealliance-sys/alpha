  ...
  return (
    <div className="shell">
      <header>
        ...
        <button className="home" onClick={() => router.push('/demo')}>Demo Home</button>
      </header>
      ...
      <main>
        ...
        <section className="programs">
          <article className="card welding">
            ...
            <button onClick={() => router.push('/demo/welding')}>Open Welding Demo</button>
          </article>

          <article className="card nursing">
            <div className="tag">HEALTH SCIENCES DEMONSTRATION</div>
            <h2>Nursing</h2>
            <p>Classroom and skills-lab instruction with protected outcomes, competency evidence, checkoffs, attendance, quality review, and program-level oversight.</p>
            <div className="chips">
          ...
          <article className="card timeclock">
            <div className="tag">OPTIONAL WORKFORCE MODULE</div>
            <h2>Finsen Sierra Time Clock</h2>
            <p>The separate LTG punch-clock module with its fixed black-and-brass timepiece face, live punch state, vintage status lamps, employee time record, and school/department branding capability.</p>
            <div className="chips">
              <span>Clock In / Out</span><span>Live Status</span><span>Time Record</span><span>Brandable</span><span>Optional Module</span>
            </div>
            <button onClick={() => router.push('/demo/time-clock')}>Open Time Clock Demo</button>
          </article>
        </section>
        ...
        <section className="message">
          ...
          <p>The core platform remains the same while each department controls its terminology, curriculum, outcomes, resources, teaching workflow, evidence model, branding, workforce modules, and reporting context.</p>
        </section>
      </main>

      <style jsx>{`
        .shell { min-height:100vh; background:#eef4f6; color:#243942; }
        header { display:flex; gap:16px; align-items:center; padding:34px max(24px,calc((100vw - 1200px)/2)); background:white; border-bottom:1px solid #d7e3e8; }
        ...
        .home { border:1px solid #cbdadf; background:#f7fafb; color:#36545f; border-radius:8px; padding:10px 13px; font-weight:850; cursor:pointer; }
        ...
        .timeclock { background:linear-gradient(145deg,#171512,#0c0d0e); color:#d8d0c4; border:1px solid #6c5233; border-top:5px solid #b48245; } .timeclock h2 { color:#f5e6cc; font-family:Georgia,'Times New Roman',serif; } .timeclock .tag { color:#d4a25f; } .timeclock p { color:#aaa093; } .timeclock .chips span { background:#211c17; border:1px solid #594329; color:#d3b889; } .timeclock button { background:#211a13; border:1px solid #b48245; color:#e1bc7d; }
        ...
        @media(max-width:980px) { .programs { grid-template-columns:1fr 1fr; } }
