...
function findSessionByCode(code: string): ClassroomRecord | null {
  const normalized = code.trim().toUpperCase();
  for (let index = 0; index < localStorage.length; index += 1) {
    const key = localStorage.key(index);
    if (!key?.startsWith('ltg_demo_classroom_')) continue;
    try {
      const record = JSON.parse(localStorage.getItem(key) || '{}') as ClassroomRecord;
      if (record.joinCode?.toUpperCase() !== normalized) continue;
      if (Date.now() - record.lastActivityAt > DEMO_TIMEOUT_MS) {
...
export default function JoinClient({ initialCode }: { initialCode: string }) {
  ...
  const join = (event?: FormEvent) => {
    ...
    if (!session) {
      setError('That demo join code is not active in this browser. Launch a Live Classroom session from the instructor demo first.');
      return;
    }
    router.push(`/demo/welding/student-display?session=${encodeURIComponent(session.id)}&activity=${encodeURIComponent(session.activityKey)}`);
  };

  useEffect(() => {
    if (!initialCode) return;
    const id = window.setTimeout(() => join(), 0);
