...
export default function AttendanceHistoryPage() {
  ...
  const queueMap = useMemo(
    () => new Map(reportQueue.map((queue) => [queue.session_id, queue])),
    [reportQueue]
  );
  ...
  const studentSummaries = useMemo(() => {
    ...
    return Array.from(byStudent.entries())
      ...
      .filter(
        (item): item is { student: Student; rows: AttendanceRecord[]; counts: SessionCounts } =>
          Boolean(item)
      )
      .filter((item) => {
        if (!normalizedSearch) return true;
  ...
  if (loading) {
    return <main className={styles.page}>Loading attendance history…</main>;
  }
    ...
    <main className={styles.page}>
      <header className={styles.header}>
        <div>
          ...
          <p>
            Review finalized and open attendance without reopening the data-entry workflow. Daily records are read-only here so historical lookup cannot accidentally change attendance.
          </p>
        </div>
      ...
      <section className={styles.viewTabs} aria-label="Attendance history view">
        <button
          type="button"
          className={view === 'daily' ? styles.activeTab : styles.tab}
        ...
        <button
          type="button"
          className={view === 'student' ? styles.activeTab : styles.tab}
          ...
          >
            ...
            {pairsForSchool.map((pair) => (
              <option key={pair.id} value={pair.id}>
                {pair.pair_name}{pair.active ? '' : ' · inactive'}
              </option>
            ))}
          </select>
        </label>
                  ...
                  <div className={styles.sessionBody}>
                    <div className={styles.metadataGrid}>
                      ...
                      <div>
                        ...
                        <strong>
                          {queue ? `${queue.status}${queue.sent_at ? ` · ${prettyDateTime(queue.sent_at)}` : ''}` : session.attendance_mode === 'pvhs' ? 'No queue record visible' : 'Not applicable'}
                        </strong>
                      </div>
                    </div>
        ...
        <section className={styles.studentWorkspace}>
          <aside className={styles.studentList}>
            <div className={styles.studentListHeader}>
              <strong>Students in selected history</strong>
          ...
          <div className={styles.studentDetail}>
            {!selectedStudent ? (
              <div className={styles.empty}>Select a student to see that student's attendance history.</div>
            ) : (
              <>
