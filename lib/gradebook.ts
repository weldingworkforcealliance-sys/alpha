...
export function gradebookStudentKey(attendanceStudentId: string | null | undefined, externalStudentId: string | null | undefined) {
...
export function categoryAverages(submissions: GradeableSubmission[]) {
  ...
  for (const category of COUNTED_GRADE_CATEGORIES) {
    const values = submissions.filter((item) => item.counts_toward_grade && item.grade_category === category).map((item) => Number(item.percent)).filter(Number.isFinite);
...
export function unweightedGradeAverage(submissions: GradeableSubmission[]) {
  return meanPercent(submissions.filter((item) => item.counts_toward_grade).map((item) => Number(item.percent)).filter(Number.isFinite));
}
...
export function currentWeightedAverage(averages: Partial<Record<CountedGradeCategory, number>>, weights: GradeWeights) {
