...
do $guard$
begin
  if md5(pg_get_functiondef('public.get_classroom_assessment(text)'::regprocedure)) <> 'cd46cb15f2846144a4e08ffc79d65f43' then
    raise exception 'Staging migration stopped: get_classroom_assessment has changed since review';
  end if;
  if md5(pg_get_functiondef('public.get_job_card_by_code(text)'::regprocedure)) <> '85e004a5cb97a4a41e09a8c350c3072b' then
    raise exception 'Staging migration stopped: get_job_card_by_code has changed since review';
  end if;
  if md5(pg_get_functiondef('public.submit_classroom_assessment_v2(text,text,text,text,jsonb)'::regprocedure)) <> '4297d58913af23dbfa1d7ecededd7b12' then
    raise exception 'Staging migration stopped: submit_classroom_assessment_v2 has changed since review';
  end if;
  if md5(pg_get_functiondef('public.submit_job_card(text,text,text,jsonb,jsonb,jsonb,text,text)'::regprocedure)) <> 'a2d5f919ebbd713edc9f46a4a5cb8e6a' then
    raise exception 'Staging migration stopped: submit_job_card has changed since review';
  end if;
end $guard$;
...
begin
  -- Bound raw input before trimming, lookup, or cleanup work.
  if p_join_code is null or octet_length(p_join_code)>128 then raise exception 'A valid class code is required'; end if;
  if char_length(btrim(p_join_code))<4 or char_length(btrim(p_join_code))>32 then raise exception 'A valid class code is required'; end if;
...
begin
  -- Bound raw input before trimming, lookup, or cleanup work.
  if p_join_code is null or octet_length(p_join_code)>128 then raise exception 'A valid Live Job Card code is required'; end if;
  if char_length(btrim(p_join_code))<4 or char_length(btrim(p_join_code))>32 then raise exception 'A valid Live Job Card code is required'; end if;
...
begin
  ...
  if jsonb_typeof(p_answers) is distinct from 'object' then raise exception 'Every question must be answered'; end if;
...
begin
  ...
  if jsonb_typeof(p_start_check) is distinct from 'object' or octet_length(p_start_check::text)>8192 then raise exception 'Invalid Start Check payload'; end if;
  if jsonb_typeof(p_quality_check) is distinct from 'object' or octet_length(p_quality_check::text)>8192 then raise exception 'Invalid Quick Quality Check payload'; end if;
  if jsonb_typeof(p_requirement_results) is distinct from 'array' or octet_length(p_requirement_results::text)>65536 then raise exception 'Invalid Job Card requirement results'; end if;
  if p_evidence_type is null or p_evidence_type not in ('none','photo','measurement','inspection_test_result') then raise exception 'Invalid evidence type'; end if;
  perform public.expire_job_card_sessions();
