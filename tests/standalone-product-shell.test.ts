...
describe('standalone LTG product shell', () => {
  it('keeps public home and demo routes separate from the authenticated workspace', () => {
  ...
  it('does not run authenticated usage tracking on public marketing or demo routes', () => {
    const layout = read('app/layout.tsx');
    expect(layout).toContain("{!isMarketingRoute && !isDemoRoute && <UsageTracker pathname={pathname} />}");
  });
