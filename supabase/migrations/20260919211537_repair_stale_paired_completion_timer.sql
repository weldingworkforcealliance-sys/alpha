-- Recover a stale prior-date completion-course timer before restarting the paired primary course.
-- Same-day reverse overlap remains blocked. Planned instructional minutes and attendance safeguards are preserved.
...
declare
  ...
  v_reverse_pair_name text;
  v_paired_completion_section_id uuid;
  v_paired_completion_planner_day_id uuid;
  v_paired_completion_day integer;
  v_paired_completion_max_days integer;
  v_paired_completion_hold boolean;
  v_paired_completion_planned_minutes integer;
  v_paired_completion_status text;
  v_paired_completion_actual_date date;
  v_paired_completion_started_at timestamptz;
  v_paired_completion_started_by uuid;
  v_paired_completion_started_by_name text;
  v_paired_completion_completed_at timestamptz;
  v_paired_completion_attendance_finalized boolean;
begin
  ...
  select ap.pair_name, ap.completion_section_id
    into v_reverse_pair_name, v_paired_completion_section_id
  from public.attendance_pairs ap
  where ap.active = true
    and ap.primary_section_id = p_section_id
  order by ap.created_at
  limit 1;

  if v_paired_completion_section_id is not null then
    select sp.current_planner_day_number,
      ...
      into v_paired_completion_day,
           v_paired_completion_max_days,
           v_paired_completion_planner_day_id,
           v_paired_completion_hold,
           v_paired_completion_planned_minutes
    from public.sections s
    join public.section_progress sp on sp.section_id = s.id
    join public.planner_days pd
      on pd.section_id = sp.section_id
     and pd.planner_day_number = sp.current_planner_day_number
    where s.id = v_paired_completion_section_id
    for update of sp;

    if v_paired_completion_planner_day_id is not null then
      select pdd.delivery_status, pdd.actual_date, pdd.started_at, pdd.instructor_id
        into v_paired_completion_status,
             v_paired_completion_actual_date,
             v_paired_completion_started_at,
             v_paired_completion_started_by
      from public.planner_day_delivery pdd
      where pdd.planner_day_id = v_paired_completion_planner_day_id
      for update;

      if found and v_paired_completion_status in ('in_progress','started') then
        v_paired_completion_actual_date := coalesce(
          v_paired_completion_actual_date,
          (v_paired_completion_started_at at time zone 'America/New_York')::date
        );

        if v_paired_completion_actual_date < p_actual_date then
          if v_paired_completion_started_at is null
             or v_paired_completion_planned_minutes is null
             or v_paired_completion_planned_minutes <= 0 then
            raise exception 'The second course in % has a stale timer from %, but LTG cannot safely repair it because its start time or planned minutes are missing.',
              coalesce(v_reverse_pair_name, 'this class pair'),
              coalesce(to_char(v_paired_completion_actual_date, 'Mon DD, YYYY'), 'an unknown date');
          end if;

          select exists(
            select 1
            from public.attendance_pairs ap
            join public.attendance_sessions ats on ats.pair_id = ap.id
            where ap.active = true
              and ap.primary_section_id = p_section_id
              and ap.completion_section_id = v_paired_completion_section_id
              and ats.attendance_date = v_paired_completion_actual_date
              and ats.status = 'finalized'
          ) into v_paired_completion_attendance_finalized;

          if not coalesce(v_paired_completion_attendance_finalized, false) then
            raise exception 'The second course in % has a stale timer from %, but attendance for that date is not finalized. Complete attendance before LTG repairs the timer.',
              coalesce(v_reverse_pair_name, 'this class pair'),
              coalesce(to_char(v_paired_completion_actual_date, 'Mon DD, YYYY'), 'an unknown date');
          end if;

          v_paired_completion_completed_at :=
            v_paired_completion_started_at
            + make_interval(mins => v_paired_completion_planned_minutes);

          update public.planner_day_delivery
          set delivery_status = 'completed',
              completed_at = v_paired_completion_completed_at,
              actual_minutes = v_paired_completion_planned_minutes,
              updated_at = now()
          where planner_day_id = v_paired_completion_planner_day_id;
          ...
          if v_paired_completion_day >= v_paired_completion_max_days then
            update public.section_progress
            set completed_at = coalesce(completed_at, v_paired_completion_completed_at),
                updated_at = now()
            where section_id = v_paired_completion_section_id;
          elsif v_paired_completion_hold then
            update public.section_progress
            ...
            update public.section_progress
            set current_planner_day_number = v_paired_completion_day + 1,
                last_advanced_at = v_paired_completion_completed_at,
                updated_at = now()
            where section_id = v_paired_completion_section_id;
          end if;
          ...
          perform public.write_audit_event(
            v_school_id,
            'automatic_stale_paired_completion_repair',
            'planner_day',
            v_paired_completion_planner_day_id,
            jsonb_build_object(
              'pair_name', v_reverse_pair_name,
              'primary_section_id', p_section_id,
              'completion_section_id', v_paired_completion_section_id,
              'completion_planner_day_number', v_paired_completion_day,
              'completion_actual_date', v_paired_completion_actual_date,
              'completion_started_by', v_paired_completion_started_by,
              'completion_started_at', v_paired_completion_started_at,
              'completion_completed_at', v_paired_completion_completed_at,
              'completion_actual_minutes', v_paired_completion_planned_minutes,
              'attendance_finalized', true,
              'repair_rule', 'prior_date_stale_timer_on_primary_start'
            )
          );
        else
          select coalesce(nullif(btrim(p.display_name), ''), p.email, 'another instructor')
            into v_paired_completion_started_by_name
          from public.profiles p
          where p.id = v_paired_completion_started_by;

          raise exception 'The second course in % is already in progress%s. Complete it before restarting the first course.',
            coalesce(v_reverse_pair_name, 'this class pair'),
            case
              when v_paired_completion_actual_date is not null
                then format(' for %s', to_char(v_paired_completion_actual_date, 'Mon DD, YYYY'))
              else ''
            end;
        end if;
      end if;
  ...
  perform public.write_audit_event(
    ...
    jsonb_build_object(
      ...
      'reverse_overlap_checked', v_paired_completion_section_id is not null
    )
  );
