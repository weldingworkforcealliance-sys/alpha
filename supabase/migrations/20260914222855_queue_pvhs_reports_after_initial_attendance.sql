...
set report_delay_minutes = 10,
    updated_at = now()
where attendance_mode = 'pvhs'
  and report_delay_minutes is distinct from 10;
  ...
  if v_inserted = 1 then
    perform public.write_audit_event(
      ...
      jsonb_build_object(
        ...
        'trigger', 'initial_attendance_complete'
      )
    );
    return true;
  end if;
...
begin
  ...
  perform private.queue_pvhs_attendance_report_if_ready(p_session_id);
end;
$function$;
