-- Prevent two active class timers within a configured course pair.
-- If the completion course is already running, the primary course cannot be restarted.
...
begin
  ...
  if v_paired_completion_section_id is not null then
    select pd.id
      into v_paired_completion_planner_day_id
    from public.section_progress sp
    join public.planner_days pd
      on pd.section_id = sp.section_id
     and pd.planner_day_number = sp.current_planner_day_number
    where sp.section_id = v_paired_completion_section_id;

    if v_paired_completion_planner_day_id is not null then
      select pdd.delivery_status, pdd.actual_date, pdd.instructor_id
