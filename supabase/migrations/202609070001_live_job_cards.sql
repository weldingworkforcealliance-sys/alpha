...
-- Prepared for push packet; do not apply until the coordinated LTG release.
...
returns table(
  ...
  title text,
  description text,
  estimated_student_minutes integer,
  default_requirement_labels jsonb,
  start_checks jsonb,
...
as $$
  select
    t.slug,t.title,t.description,t.estimated_student_minutes,
    t.default_requirement_labels,t.start_checks,t.quality_checks,t.version
  from public.job_card_templates t
  where t.active
  order by t.title
$$;
...
begin
  ...
  update public.job_card_sessions s
  set status = 'ended', ended_at = coalesce(s.ended_at,now())
...
declare
  result_id uuid;
  target_school uuid;
  template_row public.job_card_templates;
  requirement_row jsonb;
begin
  if auth.uid() is null then raise exception 'Instructor login required'; end if;
  if p_expected_students < 1 or p_expected_students > 17 then
  ...
  if jsonb_typeof(p_requirements) <> 'array'
    ...
    raise exception 'Use between 1 and 4 critical requirements';
  end if;

  for requirement_row in select value from jsonb_array_elements(p_requirements)
  loop
    if length(trim(coalesce(requirement_row->>'key',''))) < 1 then
      raise exception 'Every critical requirement needs a stable key';
    end if;
    if length(trim(coalesce(requirement_row->>'label',''))) < 2 then
      raise exception 'Every critical requirement needs a label';
    end if;
    if length(trim(coalesce(requirement_row->>'required',''))) < 1 then
      raise exception 'Every critical requirement needs a required value or N/A';
    end if;
  end loop;
  ...
  if not exists(select 1 from public.current_teaching_sections cts where cts.section_id = p_section_id)
  ...
  from public.job_card_templates t
  where t.slug = p_template_slug and t.active;
  ...
  if p_guide_day_id is not null and not exists(
    ...
    join public.sections sec on sec.course_id = d.course_id
    where d.id = p_guide_day_id and sec.id = p_section_id
  ) then
    raise exception 'Planner day does not belong to the selected class course';
  end if;

  update public.job_card_sessions s
  set status = 'ended', ended_at = now()
  where s.instructor_id = auth.uid() and s.section_id = p_section_id and s.status = 'active';

  insert into public.job_card_sessions(
    school_id,section_id,instructor_id,template_slug,guide_day_id,join_code,
    expected_students,job_header,requirements,start_checks,quality_checks
  )
  values(
    target_school,p_section_id,auth.uid(),template_row.slug,p_guide_day_id,
    public.make_job_card_join_code(),p_expected_students,p_job_header,p_requirements,
    template_row.start_checks,template_row.quality_checks
  )
...
as $$
declare
  s public.job_card_sessions;
  template_title text;
  class_label text;
begin
  select * into s
  from public.job_card_sessions jcs
  where jcs.join_code = upper(trim(p_join_code))
    and jcs.status = 'active' and jcs.expires_at > now();

  if s.id is null then raise exception 'This job card code is invalid or the session has ended'; end if;

  select t.title into template_title
  from public.job_card_templates t
  where t.slug = s.template_slug;
  ...
  return jsonb_build_object(
    'session',jsonb_build_object(
      ...
      'job_header',s.job_header,
      'requirements',s.requirements,
      'start_checks',s.start_checks,
...
create or replace function public.submit_live_job_card(
  ...
  p_requirement_checks jsonb,
  p_start_check_confirmations jsonb,
  p_quality_check_confirmations jsonb,
  p_issue_found text default null,
  p_correction text default null,
  p_recheck_status text default null,
  p_evidence_types jsonb default '[]'::jsonb,
...
declare
  ...
  check_value text;
  check_label text;
  correction_required boolean := false;
  evidence_value text;
begin
  if length(trim(coalesce(p_student_name,''))) < 2
     or length(trim(coalesce(p_student_id,''))) < 1 then
    raise exception 'Student name and ID are required';
  end if;

  if jsonb_typeof(p_actual_values) <> 'object'
     or jsonb_typeof(p_requirement_checks) <> 'object'
     or jsonb_typeof(p_start_check_confirmations) <> 'object'
     or jsonb_typeof(p_quality_check_confirmations) <> 'object'
     or jsonb_typeof(p_evidence_types) <> 'array' then
    raise exception 'Job card submission format is invalid';
  end if;
  ...
  if exists(
    select 1 from public.job_card_submissions sub
    where sub.job_card_session_id = s.id and sub.student_id = trim(p_student_id)
  ) then
    raise exception 'This Student ID has already submitted this job card';
  end if;

  for req in select value from jsonb_array_elements(s.requirements)
  loop
    req_key := req->>'key';
    ...
    if check_value not in ('pass','correct','na') then
      raise exception 'Every critical requirement must be marked Pass, Correct, or N/A';
    end if;
    if check_value <> 'na' and length(trim(coalesce(p_actual_values->>req_key,''))) < 1 then
      raise exception 'Enter the actual value for every applicable critical requirement';
    end if;
    if check_value = 'correct' then correction_required := true; end if;
  end loop;

  for check_label in select jsonb_array_elements_text(s.start_checks)
  loop
    if coalesce((p_start_check_confirmations->>check_label)::boolean,false) is not true then
      raise exception 'Complete every Start Check before submitting';
    end if;
  end loop;

  for check_label in select jsonb_array_elements_text(s.quality_checks)
  loop
    if coalesce((p_quality_check_confirmations->>check_label)::boolean,false) is not true then
      raise exception 'Complete every Quick Quality Check before submitting';
    end if;
  end loop;

  if correction_required then
    if length(trim(coalesce(p_issue_found,''))) < 2 or length(trim(coalesce(p_correction,''))) < 2 then
      raise exception 'Describe the issue and correction when a requirement needed correction';
    end if;
    if p_recheck_status not in ('pass','needs_more_work') then
      raise exception 'Record the recheck result after a correction';
    end if;
  end if;

  for evidence_value in select jsonb_array_elements_text(p_evidence_types)
  loop
    if evidence_value not in ('photo','measurement','inspection_test_result') then
      raise exception 'Evidence selection is invalid';
    end if;
  end loop;

  insert into public.job_card_submissions(
    job_card_session_id,student_name,student_id,actual_values,requirement_checks,
    start_check_confirmations,quality_check_confirmations,issue_found,correction,
    recheck_status,evidence_types,evidence_note
  )
  values(
    s.id,trim(p_student_name),trim(p_student_id),p_actual_values,p_requirement_checks,
    p_start_check_confirmations,p_quality_check_confirmations,
    nullif(trim(coalesce(p_issue_found,'')),''),nullif(trim(coalesce(p_correction,'')),''),
    case when correction_required then p_recheck_status else null end,
    p_evidence_types,nullif(trim(coalesce(p_evidence_note,'')),'')
  )
  returning id into result_id;
...
begin
  if auth.uid() is null then raise exception 'Instructor login required'; end if;
  if p_final_decision not in ('pass_move_on','continue_practice','rework_retry') then
    raise exception 'Instructor decision is invalid';
  end if;

  update public.job_card_submissions sub
  set final_decision = p_final_decision,
      instructor_note = nullif(trim(coalesce(p_instructor_note,'')),''),
      reviewed_by = auth.uid(),
      reviewed_at = now()
  from public.job_card_sessions s
  where sub.id = p_submission_id
    and s.id = sub.job_card_session_id
    and (s.instructor_id = auth.uid() or public.is_platform_owner());
...
insert into public.job_card_templates(
  slug,title,description,estimated_student_minutes,default_requirement_labels,
  start_checks,quality_checks,active,version
)
values(
  'level2_school_job_card',
  'PCCC Level II Live Job Card',
  'School-sized 3-5 minute requirement-to-evidence job card for Level II welding instruction.',
  5,
  ...
  '["Critical dimensions within requirement","Visual condition acceptable","Ready for instructor review"]'::jsonb,
  true,
  1
)
on conflict(slug) do update set
  ...
  estimated_student_minutes = excluded.estimated_student_minutes,
  default_requirement_labels = excluded.default_requirement_labels,
  start_checks = excluded.start_checks,
  quality_checks = excluded.quality_checks,
  active = true,
  version = excluded.version;

-- Make new submissions available to the instructor page through Supabase Realtime.
do $$
begin
  if not exists(
    select 1
