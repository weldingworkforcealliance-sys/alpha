...
set resource_type='website',sequence_number=1,
    ...
    resource_notes=case t.planner_day_number when 1 then 'Primary Day 1 launch. Runs the connected 20-minute diagnostic with QR join, automatic grading, live progress, and retained results.' else 'Primary teaching launch for PVHS WLD 205 Day '||t.planner_day_number||'. Use the Live Classroom as the control point for questions, progress, and results.' end,
    required=true
from target t where r.guide_day_id=t.guide_day_id and r.resource_url like '/classroom%';

with target as (
  select cgd.id as guide_day_id,cgd.planner_day_number from public.course_guide_days cgd
  join public.course_guides cg on cg.id=cgd.guide_id join public.courses c on c.id=cg.course_id
  where c.course_code='WLD 205' and cg.guide_name='WLD 205 Master Instructor Guide - Level II - PVHS Bridge + 23-Day Core + Project Extension' and cgd.planner_day_number in (3,5,6,7,8,9)
)
delete from public.course_guide_day_resources r using target t where r.guide_day_id=t.guide_day_id and r.resource_url not like '/classroom%';

with d as (
  select cgd.id,cgd.school_id,cgd.course_id,cgd.planner_day_number from public.course_guide_days cgd
  join public.course_guides cg on cg.id=cgd.guide_id join public.courses c on c.id=cg.course_id
  where c.course_code='WLD 205' and cg.guide_name='WLD 205 Master Instructor Guide - Level II - PVHS Bridge + 23-Day Core + Project Extension' and cgd.planner_day_number in (3,5,6,7,8,9)
...
insert into public.course_guide_day_resources (school_id,course_id,guide_day_id,sequence_number,resource_type,resource_title,resource_url,resource_notes,required,student_safe)
select d.school_id,d.course_id,d.id,10,'handout',v.title,v.url,v.notes,true,true from d join vals v on v.day_no=d.planner_day_number;

with target as (
  select cgd.id as guide_day_id from public.course_guide_days cgd join public.course_guides cg on cg.id=cgd.guide_id join public.courses c on c.id=cg.course_id
  where c.course_code='WLD 205' and cg.guide_name='WLD 205 Master Instructor Guide - Level II - PVHS Bridge + 23-Day Core + Project Extension' and cgd.planner_day_number=4
)
delete from public.course_guide_day_resources r using target t where r.guide_day_id=t.guide_day_id and r.resource_url='/live-activities/wld205-day4-instructor-guide.html';

with target as (
  select cgd.id as guide_day_id from public.course_guide_days cgd join public.course_guides cg on cg.id=cgd.guide_id join public.courses c on c.id=cg.course_id
  where c.course_code='WLD 205' and cg.guide_name='WLD 205 Master Instructor Guide - Level II - PVHS Bridge + 23-Day Core + Project Extension' and cgd.planner_day_number=4
)
update public.course_guide_day_resources r set resource_type='handout',resource_title='Day 4 Decimal Student Pack',sequence_number=10,required=true,resource_notes='Single Day 4 practice packet for fraction/decimal retrieval, Decimal Shop Set A, measurement record, and rounding/tolerance preview.' from target t where r.guide_day_id=t.guide_day_id and r.resource_url='/live-activities/wld205-day4-decimal-student-pack.html';

update public.course_guide_days cgd set materials_equipment=case planner_day_number
...
where cgd.guide_id=cg.id and cg.course_id=c.id and c.course_code='WLD 205' and cg.guide_name='WLD 205 Master Instructor Guide - Level II - PVHS Bridge + 23-Day Core + Project Extension' and cgd.planner_day_number between 1 and 9;
