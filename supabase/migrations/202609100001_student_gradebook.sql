...
create index if not exists classroom_submissions_attendance_student_idx
  on public.classroom_submissions(attendance_student_id);
create index if not exists classroom_sessions_section_started_grade_idx
  on public.classroom_sessions(section_id, started_at desc, grade_category);
create index if not exists job_card_submissions_attendance_student_idx
  on public.job_card_submissions(attendance_student_id);

create table if not exists private.course_grade_settings (
  course_id uuid primary key references public.courses(id) on delete cascade,
  school_id uuid not null references public.schools(id) on delete cascade,
  weights jsonb not null default '{}'::jsonb,
  calculation_method text not null default 'mean_percent'
    check (calculation_method in ('mean_percent')),
  updated_by uuid references auth.users(id) on delete set null,
  updated_at timestamptz not null default clock_timestamp(),
  constraint course_grade_settings_weights_object
    check (jsonb_typeof(weights) = 'object')
);

revoke all on table private.course_grade_settings from public, anon, authenticated;

create or replace function private.resolve_attendance_student_for_section(
  p_school_id uuid,
  p_section_id uuid,
...
declare
  ...
  v_section_id uuid;
begin
  select s.school_id, s.section_id
    into v_school_id, v_section_id
  from public.classroom_sessions s
  where s.id = new.classroom_session_id;

  if v_school_id is not null then
    new.attendance_student_id := private.resolve_attendance_student_for_section(
      v_school_id,
      v_section_id,
      new.student_id
    );
...
begin
  ...
  where s.id = new.job_card_session_id;
...
create trigger classroom_submission_link_student
before insert or update of classroom_session_id, student_id
on public.classroom_submissions
for each row execute function private.link_classroom_submission_student();

drop trigger if exists job_card_submission_link_student on public.job_card_submissions;
create trigger job_card_submission_link_student
before insert or update of job_card_session_id, student_id
on public.job_card_submissions
for each row execute function private.link_job_card_submission_student();

-- Backfill only unambiguous roster matches. Unmatched historical submissions remain intact.
update public.classroom_submissions sub
set attendance_student_id = private.resolve_attendance_student_for_section(
  sess.school_id,
  sess.section_id,
  sub.student_id
)
from public.classroom_sessions sess
where sess.id = sub.classroom_session_id
  and sub.attendance_student_id is null;
...
where sess.id = sub.job_card_session_id
  and sub.attendance_student_id is null;
...
declare
  v_id uuid;
  v_category text := lower(btrim(coalesce(p_grade_category,'practice_only')));
begin
  if auth.uid() is null then
    raise exception 'Instructor login required';
  end if;

  if v_category not in ('practice_only','test','quiz','task','activity','practical','competency') then
    raise exception 'Invalid grade category';
  end if;

  v_id := public.start_classroom_session_v2(
    p_section_id,
    p_assessment_slug,
    p_expected_students
  );
  ...
  set grade_category = v_category,
      counts_toward_grade = v_category in ('test','quiz','task','activity','practical')
  where id = v_id
...
create or replace function public.set_classroom_session_grade_category(
  p_session_id uuid,
  p_grade_category text
)
returns void
language plpgsql
security definer
...
begin
  ...
  if v_session.instructor_id <> auth.uid()
     and not public.can_manage_school(v_session.school_id)
...
create or replace function public.save_course_grade_settings(
  p_course_id uuid,
  p_weights jsonb
)
returns jsonb
...
begin
  ...
  if not public.can_manage_school(v_school_id) and not public.is_platform_owner() then
    raise exception 'School or program administrator access required';
  end if;

  if jsonb_typeof(v_weights) <> 'object' then
    raise exception 'Grade weights must be an object';
  end if;

  for v_key, v_value in select key, value from jsonb_each(v_weights)
  loop
    if v_key not in ('test','quiz','task','activity','practical') then
      raise exception 'Unsupported grade category: %', v_key;
    end if;
    if jsonb_typeof(v_value) <> 'number' then
      raise exception 'Each grade weight must be numeric';
    end if;
    if (v_value #>> '{}')::numeric < 0 or (v_value #>> '{}')::numeric > 100 then
      raise exception 'Each grade weight must be between 0 and 100';
    end if;
    v_total := v_total + (v_value #>> '{}')::numeric;
  end loop;

  if v_weights <> '{}'::jsonb and v_total <> 100 then
    raise exception 'Grade weights must total 100 percent';
  end if;
...
begin
  ...
  if not v_can_manage
     and not public.is_section_instructor(v_school_id,p_section_id) then
    raise exception 'You do not have permission to view this gradebook';
  end if;

  select coalesce(g.weights,'{}'::jsonb)
    into v_weights
  from private.course_grade_settings g
  ...
  select jsonb_build_object(
    ...
    'roster', coalesce((
      ...
      from (
        select distinct jsonb_build_object(
          'attendance_student_id', st.id,
          'display_name', st.display_name,
          'external_student_id', st.external_student_id
        ) as r
        from public.attendance_pairs p
        join public.attendance_pair_enrollments e
          on e.pair_id=p.id and e.school_id=p.school_id and e.active=true
        join public.attendance_students st
          on st.id=e.student_id and st.school_id=p.school_id and st.active=true
    ...
    'sessions', coalesce((
      select jsonb_agg(jsonb_build_object(
        'session_id',sess.id,
        'assessment_slug',sess.assessment_slug,
        'assessment_title',coalesce(m.title,sess.assessment_slug),
        'grade_category',sess.grade_category,
        'counts_toward_grade',sess.counts_toward_grade,
        'status',sess.status,
      ...
      from public.classroom_sessions sess
      left join public.assessment_modules m on m.slug=sess.assessment_slug
      where sess.section_id=p_section_id
    ),'[]'::jsonb),
    'submissions', coalesce((
      select jsonb_agg(jsonb_build_object(
        'submission_id',sub.id,
        'session_id',sess.id,
        'attendance_student_id',sub.attendance_student_id,
        ...
        'percent',round(100.0*sub.score/greatest(sub.possible_score,1),1),
        'domain_scores',sub.domain_scores,
        'submitted_at',sub.submitted_at
      ) order by sub.submitted_at desc)
      from public.classroom_submissions sub
      join public.classroom_sessions sess on sess.id=sub.classroom_session_id
      left join public.assessment_modules m on m.slug=sess.assessment_slug
      where sess.section_id=p_section_id
    ),'[]'::jsonb),
    'job_card_evidence', coalesce((
      select jsonb_agg(jsonb_build_object(
        'submission_id',sub.id,
        'attendance_student_id',sub.attendance_student_id,
        'student_name',sub.student_name,
        ...
        'planner_day_number',sess.planner_day_number,
      ...
      join public.job_card_sessions sess on sess.id=sub.job_card_session_id
      where sess.section_id=p_section_id
    ),'[]'::jsonb)
  ) into v_result;
