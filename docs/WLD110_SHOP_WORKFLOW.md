...
Apply the four `wld110_shop_*` migrations in filename order, then enable
...
Before production activation, extend the separately pinned archive worker's
reviewed inventory and reader grants to include `wld110_shop_progress`,
`wld110_shop_attempts`, and `wld110_shop_completions`, and verify recovery.
The currently deployed archive worker retains posted gradebook scores but does
not yet include these new detailed shop records. Private student access tokens
must remain excluded. This PR preserves the existing archive configuration.
